...
pte_t find_va(pte_t pte, int idx, int level) {
  uint64 I = (uint64) idx;
  return pte | (I<<(12+9*(2-level)));
}

void re_vmprint(pte_t* P, int idx, int level, int last_val, pte_t va) {
  pte_t pte = *P;
  for(int i = 0; i < level; i++) {
    // printf("%s\n", prefix_str);
    int tmp = prefix_str[i];
    if(tmp == 0) printf("    ");
    else printf("│   ");
  }
