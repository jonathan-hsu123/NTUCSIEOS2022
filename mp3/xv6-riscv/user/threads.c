...
void schedule(void){
    ...
    if(is_thread_start == 0 || current_thread -> is_yield || current_thread -> is_exited) {
        ...
        if(current_thread -> is_exited) next = current_thread -> next;
        for(struct thread* tmp_thread = current_thread -> next; tmp_thread != current_thread; tmp_thread = tmp_thread -> next) {
